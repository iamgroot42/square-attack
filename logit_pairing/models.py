...
class LeNet:
    def __init__(self):
        ...
        # Create variables
        with tf.compat.v1.variable_scope('conv1_vars'):
        ...
        with tf.compat.v1.variable_scope('conv2_vars'):
        ...
        with tf.compat.v1.variable_scope('fc1_vars'):
        ...
        with tf.compat.v1.variable_scope('fc2_vars'):
        ...
        with tf.compat.v1.name_scope('conv-block-1'):
        ...
        with tf.compat.v1.name_scope('max-pool-1'):
            conv1 = tf.nn.max_pool2d(input=conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')

        with tf.compat.v1.name_scope('conv-block-2'):
        ...
        with tf.compat.v1.name_scope('max-pool-2'):
            conv2 = tf.nn.max_pool2d(input=conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')

        with tf.compat.v1.name_scope('fc-block'):
            conv2 = tf.compat.v1.layers.flatten(conv2)
...
class ResNet20_v2:
    def __init__(self):
        ...
        with tf.compat.v1.variable_scope('conv10_vars'):
        ...
        with tf.compat.v1.variable_scope('conv15_vars'):
        ...
        with tf.compat.v1.variable_scope('scip1_vars'):
        ...
        with tf.compat.v1.variable_scope('scip2_vars'):
        ...
        with tf.compat.v1.variable_scope('scip3_vars'):
        ...
        self.x_input = tf.compat.v1.placeholder(tf.float32, shape=[None, 32, 32, 3])
        self.y_input = tf.compat.v1.placeholder(tf.int64, shape=None)
        x = self.x_input / 255.0

        # Specify forward pass
        with tf.compat.v1.name_scope('input-block'):
        ...
        with tf.compat.v1.name_scope('conv-block-1'):
            ...
            # skip connection
            conv3 += tf.nn.conv2d(input=conv1, filters=self.W_scip1, strides=[1, 1, 1, 1], padding='SAME', name='conv-skip1')
        ...
        with tf.compat.v1.name_scope('conv-block-2'):
            ...
            # skip connection
            conv9 += tf.nn.conv2d(input=conv7, filters=self.W_scip2, strides=[1, 2, 2, 1], padding='SAME', name='conv-skip2')
        ...
        with tf.compat.v1.name_scope('conv-block-3'):
            ...
            # skip connection
            conv15 += tf.nn.conv2d(input=conv13, filters=self.W_scip3, strides=[1, 2, 2, 1], padding='SAME', name='conv-skip3')
        ...
        with tf.compat.v1.name_scope('output-block'):
            with tf.compat.v1.name_scope('global-average-pooling'):
                fc1 = tf.reduce_mean(input_tensor=conv19, axis=[1, 2])
...
def create_weights(units_in, units_out, init='Xavier', seed=None):
    if init == 'Xavier':
        initializer = tf.compat.v1.variance_scaling_initializer(scale=1.0,
                                                      mode='fan_in',
                                                      distribution='normal',
                                                      seed=None,
                                                      dtype=tf.float32)
    elif init == 'He':
        initializer = tf.compat.v1.variance_scaling_initializer(scale=2.0,
                                                      mode='fan_in',
                                                      distribution='normal',
                                                      seed=None,
                                                      dtype=tf.float32)
    else:
        initializer = tf.compat.v1.truncated_normal_initializer(mean=0.0, stddev=0.01, seed=seed, dtype=tf.float32)
...
def create_biases(size):
    return tf.compat.v1.get_variable(name='biases', shape=[size], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer())
...
def conv_layer(inputs, is_training, weights, stride, padding, bn, bn_scale, bn_bias, name,
    ...
    if bias is not None:
        inputs = tf.nn.conv2d(input=inputs, filters=weights, strides=[1, stride, stride, 1], padding=padding) + bias
    else:
        inputs = tf.nn.conv2d(input=inputs, filters=weights, strides=[1, stride, stride, 1], padding=padding)
...
def pre_act_conv_layer(inputs, is_training, weights, stride, padding, bn, bn_scale, bn_bias, name,
    ...
    outputs = tf.nn.conv2d(input=activations, filters=weights, strides=[1, stride, stride, 1], padding=padding)
